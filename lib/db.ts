...
export async function dbConnect(forceRefresh = false): Promise<typeof mongoose> {
  ...
  if (forceRefresh || !cached.conn || now - cached.lastRefresh > refreshInterval) {
    ...
    // Clear the IssuerRequest model cache to prevent schema conflicts
    if (mongoose.models.IssuerRequest) {
      console.log("Clearing cached IssuerRequest model");
      delete mongoose.models.IssuerRequest;
    }
...
// issuer request had issue so i changed code , below is old code , its also working , 
// import mongoose from "mongoose";
